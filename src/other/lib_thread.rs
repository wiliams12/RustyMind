...
pub fn negamax_root(board: &Board, depth: i32) -> Option<ChessMove> {
        match board.status() {
        ...
        // ! The functions don't work as expected, the error might not be in the threading

        // let mut lines = Vec::new();
        // let (tx, rx) = mpsc::channel();
        for mv in reorder_moves(&board, MoveGen::new_legal(&board)) {
            //lines.push(thread::spawn({
            //    let board_clone = board.clone();
            //    let tx = tx.clone();
            //    move || {
//
            //        let score = -negamax(&board_clone, -99999, 99999, depth - 1);
            //        tx.send((score, mv)).unwrap();
            //    }
            //}));
            let mut eval = -negamax(&board, -99999, 99999, depth - 1);
            let mut random = thread_rng();
            let random = random.gen_range(-1..=1);
            eval += random;
            if eval > max_eval {
                max_eval = eval;
                best_move = Some(mv);
        ...
        //     let mut random = thread_rng();
        //     let random = random.gen_range(-1..=1);
        //     let eval = res.0 + random;
        //     if eval > max_eval {
        //         max_eval = eval;
        //         best_move = Some(res.1);
        //     }
        // }
        // for line in lines {
        //     line.join().unwrap();
     ...
     fn negamax(board: &Board, mut alpha: i32, beta: i32, depth: i32) -> i32 {
        ...
        for mv in reorder_moves(&board, MoveGen::new_legal(&board)) {
            let eval = -negamax(&board.make_move_new(mv),-beta,-alpha,depth-1);
            if alpha >= beta{
                break
            }
    ...
    fn quiescence_search(board: &Board, mut alpha: i32, beta: i32, depth: i32) -> i32 {
        ...
        for mv in filter_moves(board, MoveGen::new_legal(&board), depth) {
            let eval = -quiescence_search(&board.make_move_new(mv),-beta,-alpha, depth+1);
            if eval >= beta{
                return beta;
            }
    ...
    fn evaluation(board: &Board, alpha: i32, beta: i32) -> i32 {
        match board.status() {
        ...
        if eval > beta - margin || eval < alpha + margin {
            ...
            // TODO use center only if cache is on
            // TODO If cache is on, implement pawn structure
            eval += mobility(board, Color::White); // + self.center_control(board, Color::White);
            eval -= mobility(board, Color::Black); // + self.center_control(board, Color::Black);
    ...
    fn sorting_func(board: &Board, chess_move: &ChessMove) -> i32 {
    ...
    fn reorder_moves(board: &Board, moves: MoveGen) -> Vec<ChessMove>{
    ...
    fn filter_moves(board: &Board, moves: MoveGen, depth: i32) -> Vec<ChessMove> {
