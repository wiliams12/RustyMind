...
pub fn id(console: &mut io::StdoutLock) {
    writeln!(console, "id name Rusty Mind 1.0").unwrap_or_else(|_| panic!("Error writing to the standard output"));
    writeln!(console, "id author Vilém Učík").unwrap_or_else(|_| panic!("Error writing to the standard output"));
}
...
pub fn set_depth(game: &mut Game, input: &str, console: &mut io::StdoutLock) {
    ...
    if words.len() > 5 || value < 1 || value > 6{
